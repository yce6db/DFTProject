...
#lat_values = np.linspace(3.5,4.5,11)
#olumes = []
#energies = []
#for lat in lat_values:
   # atoms = bulk("Al", "fcc", a=lat)
...
#a0 = (4*v0)**(1/3)
#B_GPa = B / kJ * 1e24
#atoms_bulk = bulk("Al", "fcc", a=a0)
#atoms_bulk.set_calculator(calc)
#energy_bulk = atoms_bulk.get_potential_energy()
#atom_individual = Atoms("Al", positions=[[0, 0, 0]],pbc=False)
...
#print(f"Cohesive energy: {cohesive_energy:.4f} eV")
def calculate_bulk_and_lattice_constant(element,structure,lattice_prediction):
    ...
    B_GPa = B / kJ * 1e24
    return a0, B_GPa
print(calculate_bulk_and_lattice_constant("AlP","zincblende",5.458))
